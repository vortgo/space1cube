...
inline CubePos getNeighbor(int face, int x, int y, int dx, int dy) {
    ...
    if (face == 0) { // FRONT
        ...
        else if (newY >= H) { result = {4, W-1-x, H-1}; } // → top (180° rotated, enter from y=7)
        else if (newY < 0) { result = {5, W-1-x, 0}; }    // → bottom (180° rotated, enter from y=0)
    }
    else if (face == 2) { // LEFT
        if (newX < 0) { result = {1, W-1, y}; }           // → back
        else if (newX >= W) { result = {0, 0, y}; }       // → front
        else if (newY >= H) { result = {4, 0, H-1-x}; }   // → top (rotated)
        else if (newY < 0) { result = {5, 0, x}; }        // → bottom (rotated)
    }
    else if (face == 1) { // BACK
        if (newX < 0) { result = {3, W-1, y}; }           // → right
        else if (newX >= W) { result = {2, 0, y}; }       // → left
        else if (newY >= H) { result = {4, x, 0}; }       // → top (enter from y=0)
        else if (newY < 0) { result = {5, x, H-1}; }      // → bottom (enter from y=7)
    }
